/*
 * Internal-link check over the emitted site (run after the build; CI runs
 * it before publishing). Every <base>/... href/src/data-knowl in every
 * emitted page must resolve inside the built site (config.siteDir): a
 * slashed URL to <path>/index.html, anything else to a literal file.
 * Relative refs are an error by construction — the site's URLs are
 * root-relative (plans/bhsawesome-index-html-urls.md) — as is any
 * bare-#fragment whose id is missing from the page itself.
 */
...
/** Returns the problem list (empty = clean) after printing each problem. */
export function checkLinks(config: BookConfig): string[] {
  setConfig(config);
  const SITE = config.siteDir;
  const BASE = config.base;
  // Known-missing refs (config allowlist): the gate stays red for NEW
  // breakage; remove entries as the targets are sourced or the refs dropped.
  const knownMissing = new Set(config.knownMissing ?? []);
  ...
  for (const file of htmlFiles(SITE)) {
    ...
    for (const m of html.matchAll(REF_RE)) {
      ...
      if (knownMissing.has(ref)) continue;
      checked += 1;
      if (ref.startsWith('#')) {
