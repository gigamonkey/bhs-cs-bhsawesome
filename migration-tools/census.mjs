...
const OVERRIDES = new Map([
  ...
  // 'para' left the vocabulary at step 2 (2026-08-09): paragraphs are
  // plain <p>; CSS keys on p[id] where our paragraphs must be told apart
  // from runestone-generated ones. The name survives only in
  // pretext-core's generated knowl-error markup and prefix-919's now-dead
  // MCQ rule (book.css carries the label>p replacement).
...
const NOTES = new Map([
  ...
  ['autopermalink', 'injected at load by builder/permalinks.js (step 9 — not in static HTML); consumed by pretext-core (copy/expose machinery)'],
  ...
  ['punctuation', 'Prism token, deliberately unstyled: the old theme colored it identically to the base code color in both modes (a no-op), so the phase-5 extraction dropped it'],
...
const lines = [];
lines.push('# BHSawesome class audit — the CSS-cleanup ledger');
lines.push('');
lines.push('The class-consumer ledger for the book (born with the css-cleanup');
lines.push('plan, done/bhsawesome-css-cleanup.md). Generated by');
lines.push('`node migration-tools/census.mjs` **in the bhs-cs-bhsawesome repo**');
lines.push('(hand judgments live in that script’s OVERRIDES/NOTES maps — edit');
lines.push('there, not here). Re-run after each cleanup step; the burn-down shows');
lines.push('up in the counts below.');
lines.push('');
